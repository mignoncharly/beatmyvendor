...
export default async function SoftwarePage({ params }: Props) {
  ...
  const schema = { "@context": "https://schema.org", "@type": "SoftwareApplication", name: product.name, applicationCategory: "BusinessApplication", operatingSystem: "Web", url: product.website, description: product.summary };
  return <div className={s.page}><script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify(schema) }} /><section className={s.hero}><nav className={s.breadcrumbs}><Link href="/">Home</Link> / Software / {product.name}</nav><span className="eyebrow">Customer support software</span><h1>Get a better {product.name} deal.</h1><p>{product.summary} If you already use it, your verified spend can invite credible competitors to submit private replacement offers.</p><div className={s.actions}><Link className="button button-primary" href="/start">Challenge your deal →</Link><Link className="button button-secondary" href={`/alternatives/${slug}`}>See alternatives</Link></div></section><section className={s.section}><span className="eyebrow">Common alternatives</span><h2>Who could beat {product.name}?</h2><div className={s.grid}>{alternatives.map((item) => <article className={s.card} key={item.slug}><span className={s.number}>Alternative</span><h3>{item.name}</h3><p>{item.summary}</p><Link href={`/compare/${slug}-vs-${item.slug}`}>Compare →</Link></article>)}</div></section></div>;
}
